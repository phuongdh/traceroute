...
void doStuff(int acceptfd, struct sockaddr_in cliaddr, pid_t pid) {
	...
	if (curUsers > numUsers) {
		...
		log(
				"cleint disconnected due to max number of users error occurred while reading data from client");
		shutdown(acceptfd, 2);
		_exit(EXIT_FAILURE);
	}
	...
	for (;;) {
		...
		while (1 == 1) {
			...
			if (command == NULL ) {
				...
				log(" 'help' command issued");
				system(
						"echo \"server start up: <port number> <number requests> <number seconds> <number of users> <0 or1>\"");
				system(
				...
				log(" 'traceroute me' command issued");
...
void execute(char * command, char * ipaddress, char * hostname) {
	...
	if (dest == 1) {
		...
		if (i != 0 && j != 0) {
			...
			log(
					" users are only allowed to send traceroutes to their own IP addresses when STRICT_DEST is true. discarded the command.  ");
